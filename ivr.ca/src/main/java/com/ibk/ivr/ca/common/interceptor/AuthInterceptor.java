...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(as == null) {
			...
			if(!"1".equals(as.getAuthyGrpCd())) {
				log.info("auth requst addr {}", request.getRemoteAddr());
				log.info("auth requst URI {}", request.getRequestURI());
				log.info("auth requst failed name : {}, nr : {}", as.getEmpNm(), as.getEmpNr());
				return sendError(request, response, handler);
			}
		}
		return true;
