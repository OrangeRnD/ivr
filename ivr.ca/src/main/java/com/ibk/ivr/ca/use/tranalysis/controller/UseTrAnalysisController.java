package com.ibk.ivr.ca.use.tranalysis.controller;
...
import com.ibk.ivr.ca.use.tranalysis.service.UseTrAnalysisService;

import lombok.extern.slf4j.Slf4j;

/**
 * 실시간 거래 발생 비교 controller
 * 
...
@Slf4j
@Controller
@RequestMapping(value = "/use/tranalysis", method=RequestMethod.POST)
public class UseTrAnalysisController {

	@Autowired
	private UseTrAnalysisService service;
	...
	public String index() throws Exception {
		return "/index/use/tranalysis/tr_analysis_index";
	}
	...
	public @ResponseBody ResponseVO selectList(@ModelAttribute RequestVO requestVO, HttpSession session) throws Exception {
		...
		((UsrVO)session.getAttribute(Constants.SESSION)).getParam().put("use_tranalysis", requestVO.getParam());
	...
	public AbstractXlsxView selectListExcel(@ModelAttribute RequestVO requestVO, Model model) throws Exception {
		...
		model.addAttribute(Constants.FILE_NAME, "실시간거래발생비교");
		
		return new DefaultXlsxView() {

			@Override
			protected void create(Map<String, Object> model, HttpServletRequest request) throws Exception {
				Sheet sheet = workbook.createSheet("실시간 거래 발생 비교");
				sheet.setDefaultColumnWidth(15);
				
		        short r = 1;
				this.createTitle(sheet, r++, "실시간 거래 발생 비교");

                StringBuilder sb = new StringBuilder("");
				String[] titles = {"시간", "거래 건수", "거래 건수"};
				String[] titles2 = {"시간", "거래 건수", "거래 건수", ""};
