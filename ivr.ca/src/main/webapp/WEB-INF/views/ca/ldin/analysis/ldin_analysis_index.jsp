...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		<div class="titleBox">
			<p class="ltit">실시간 인입추이 비교</p>
		</div>
		<div class="tblBox">
			<form data-type="search" name="search_frm" data-session-type="${sessionScope.session.param.ldin_analysis.TYPE}">
			<input type="hidden" name="param[DT]" value="${sessionScope.session.param.ldin_analysis.DT}"/>
			<input type="hidden" name="param[FROM_DT]" value="${sessionScope.session.param.ldin_analysis.FROM_DT}" data-type="formated"/>
			<input type="hidden" name="param[TO_DT]" value="${sessionScope.session.param.ldin_analysis.TO_DT}" data-type="formated"/>
			<input type="hidden" name="param[PRE_DT]" value="${sessionScope.session.param.ldin_analysis.PRE_DT}" data-type="formated"/>
			...
			<table class="tbltype01">
				...
				<tr>
					<th>기준일</th>
					<td>
						<input id="RESTDE_ALTV" type="radio" name="param[RESTDE_ALTV]" value="-1" data-search="true"<c:if test="${sessionScope.session.param.ldin_analysis.RESTDE_ALTV == null || sessionScope.session.param.ldin_analysis.RESTDE_ALTV == '-1'}"> checked</c:if>><label for="RESTDE_ALTV"><span></span>전체</label>
						<input id="RESTDE_ALTV0" type="radio" name="param[RESTDE_ALTV]" value="0" data-search="true"<c:if test="${sessionScope.session.param.ldin_analysis.RESTDE_ALTV == '0'}"> checked</c:if>><label for="RESTDE_ALTV0"><span></span>영업일만</label>
						<input id="RESTDE_ALTV1" type="radio" name="param[RESTDE_ALTV]" value="1" data-search="true"<c:if test="${sessionScope.session.param.ldin_analysis.RESTDE_ALTV == '1'}"> checked</c:if>><label for="RESTDE_ALTV1"><span></span>휴일만</label>
					</td>
					<th>상하위 제외</th>
					<td>
						<span class="label">상하위</span><input type="text" class="input01 int" name="param[AVG_ABOVE]" data-type="int" maxlength="2" data-format="true" value="${sessionScope.session.param.ldin_analysis.AVG_ABOVE}"/> <span class="label">%</span>
					</td>
				</tr>
			</table>
			<div class="btnArea">
				...
				<a class="btntype02 btn_primary" id="btn_excel hide" data-url="/ldin/analysis/excel">엑셀</a>
				</div>
			</div>
			</form>
		</div>
		...
		<div class="chartWrap">
			<div class="chTit">
				<span class="mtit">실시간 콜 인입 현황</span>
